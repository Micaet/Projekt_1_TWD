...
Wszystkie <- read.csv("tmp1.csv") %>% filter (!Deaths == "")
Samobojstwa_Stany <- read.csv("tmp2.csv") %>%
  filter(Deaths != "") %>%                                
  mutate(
...
Narkotyki_Stany <- read.csv("tmp3.csv") %>%
  filter(Deaths != "") %>%
  mutate(
    Population = Population %>% 
      str_replace_all(",", "") %>%
...
Pojazdy_Stany <- read.csv("tmp5.csv") %>%
  filter(Deaths != "") %>%
  mutate(
    Population = Population %>% 
      str_replace_all(",", "") %>%   
...
Upadki_Stany <- read.csv("tmp6.csv") %>%
  filter(Deaths != "") %>%
  mutate(
    Population = Population %>% 
      str_replace_all(",", "") %>%
...
Przestepstwa_Stany <- read.csv("tmp9 (2).csv") %>%
  filter(Deaths != "") %>%
  mutate(
    Population = Population %>% 
      str_replace_all(",", "") %>% 
...
radarchart(radar_data,
           ...
           cglwd = 0.8,                            
           caxislabels = c("", "", "", "", ""),  
           title = "Costs of fatalities per capita in USD by region",
           maxmin = TRUE                   
)
legend("topright",
